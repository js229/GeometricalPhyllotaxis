...
uv = euclideanUV[mn];
 str = StringTemplate["``*``-``*``=``"][mn[[1]],uv[[2]],mn[[2]],uv[[1]],mn[[1]]*uv[[2]]-mn[[2]]*uv[[1]]];
{str,euclideanDelta[mn]}
];
...
*)
Echo[m,"lPL"];
itable= latticeParastichyPointsThrough[lattice,m];
Echo[itable,"lPL"];
Map[latticeParastichyLines[lattice,m,#]&,itable]
];
...
pvecm = latticePoint[lattice,m];thr = latticePoint[lattice,through];cylinder = latticeGetCylinder[lattice];cylinderLU = cylinder[[2]];If[zeroParastichyQ[m],
v = latticeParastichyVerticalSeparation[lattice,0];line = {{cylinder[[1,1]],v*through},{cylinder[[1,2]],v*through}},(* or *) 
If[pvecm[[1]]==0,
v = latticeParastichyHorizontalSeparation[lattice,1];line ={ {v*through,cylinder[[2,1]]},{v*through,cylinder[[2,2]]}},
(* or general case *) 
pvecslope = pvecm[[2]]/pvecm[[1]];bottom = { (cylinderLU[[1]]-thr[[2]])/pvecslope +thr[[1]], cylinderLU[[1]]};top        = { (cylinderLU[[2]]-thr[[2]])/pvecslope+thr[[1]],cylinderLU[[2]]};
line = {};
lastXZ = bottom;
i=0;
While[
nextX = If[pvecslope>0, cylinder[[1,2]],cylinder[[1,1]] ];
nextZ = lastXZ[[2]]+(nextX-lastXZ[[1]])* pvecslope;
