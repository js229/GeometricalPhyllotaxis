...
makeEuclideanProductMatrix[q_] := Module[{A,P,qfunc},
qfunc[i_] := q[[i+1]]; (* qfunc indexes from 0 *)
A[i_] := {{ 0,1},{1,-qfunc[i]}};
P = IdentityMatrix[2];
...
uv = euclideanUV[mn];
 str = StringTemplate["``*``-``*``"][mn[[1]],uv[[2]],mn[[2]],uv[[1]]];
{str,ToExpression[str],euclideanDelta[mn]}
];
...
numberate[pvectors_, h_] := Module[{rawpnumber,pnumbers,onepos,hatpos},
rawpnumber[v_] :=  Abs[Round[v[[2]]/h ]];
pnumbers = Map[rawpnumber,pvectors];
onepos = Position[pnumbers,1];
If[Length[onepos]>= 2, hatpos = onepos[[2]]; pnumbers[[hatpos]] = hat[1]];
Association[Map[#[[1]]->#[[2]] &,Transpose[{pnumbers,pvectors}]]]
];
...
latticePoints[lattice_] :=  Module[{nmin,nmax,irange,points,h,cylinderLU},
h = lattice["h"];cylinderLU=lattice["cylinder"][[2]];
{nmin,nmax}= {Ceiling[Min[cylinderLU]/h],Floor[Max[cylinderLU]/h]};
irange = {nmin,nmax};
points = N@Table[latticePoint[lattice,i],{i,nmin,nmax}];
points
];
...
pvector[{d_,h_},m_] := If[zeroParastichyQ[m],{1,0},lpoint[{d,h},m]];latticePoint[lattice_,m_] :=lpoint[{lattice["d"],lattice["h"]},m];
...
latticeVectorLength[lattice_,m_] := Module[{mEnd= latticeVector[lattice,m]}, Sqrt[mEnd . mEnd]];
...
(* use funcs above ...*) 
latticeParastichyRange[lattice_,m_] :=  Module[{multiple,parastichyLine,arena,ilowerB, iupperB,translationD},
arena = latticeGetCylinder[lattice];
If[zeroParastichyQ[m],
translationD = latticeParastichyVerticalSeparation[lattice,0];
ilowerB = -Floor[(  - arena[[2,1]])/translationD];
iupperB = Floor[(arena[[2,2]])/ translationD];
, (* or a slope *) 
...
{{  arena[[1,1]], arena[[2,2]]},{ arena[[1,2]],arena[[2,2]]}},
{ {0,0},  latticeVector[lattice,m]}][[1]];
ilowerB = -Floor[( arenaBottomIntersection - arena[[1,1]])/translationD];
iupperB = Floor[(arena[[1,2]]- arenaBottomIntersection )/ translationD]
];
...
(* ::Input::Initialization:: *)
latticeParastichyLines[lattice_,m_] :=  Module[{ilower,iupper },
{ilower,iupper}= latticeParastichyRange[lattice,m];
Table[latticeParastichyLines[lattice,m,i]
,{i,ilower,iupper}]
];


latticeParastichyLines[lattice_,m_,through_] := Module[{pvecm,thr,pvecslope,bottom,top,wixrange,wixseq,xl,xr,xlr,xwlr,zofxfunc,zwlr,cylinder,cylinderLU,line},

pvecm = latticePoint[lattice,m];
thr = latticePoint[lattice,through];
cylinder = latticeGetCylinder[lattice];
cylinderLU = cylinder[[2]];
If[zeroParastichyQ[m],
v = latticeParastichyVerticalSeparation[lattice,0];
line = {{cylinder[[1,1]],v*through},{cylinder[[1,2]],v*through}},
(* or *) 
If[pvecm[[1]]==0,
v = latticeParastichyHorizontalSeparation[lattice,1];
line ={ {v*through,cylinder[[2,1]]},{v*through,cylinder[[2,2]]}},
(* or general case *) 
pvecslope = pvecm[[2]]/pvecm[[1]];
bottom = { (cylinderLU[[1]]-thr[[2]])/pvecslope +thr[[1]], cylinderLU[[1]]};
top        = { (cylinderLU[[2]]-thr[[2]])/pvecslope+thr[[1]],cylinderLU[[2]]};
If[pvecslope<0, 
{top,bottom} = {bottom, top}
]; (* split the x-range into all the different winding numbers and return one pair of endpoints for each segment *)
wixrange = Floor[0.5+{First[bottom],First[top]}];
If[Max[wixrange]-Min[wixrange]>100,
Print["Too many parastichies"];Abort[]];
wixseq = Range[wixrange[[1]],wixrange[[2]]];
xl = wixseq - 1/2;
xr = wixseq + 1/2;
xlr = Transpose[{xl,xr}];
xlr[[1,1]] = bottom[[1]];xlr[[-1,2]]=top[[1]];
xwlr =  xlr-Transpose[{wixseq,wixseq}];
zofxfunc := ( Interpolation[{ bottom, top}, InterpolationOrder->1]);
zwlr = Map[ zofxfunc,xlr];
line = Partition[Transpose@{Flatten@xwlr,Flatten@zwlr},2]
]];
Line[line]
];
